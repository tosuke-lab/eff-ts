...
export class EffError {
  constructor(readonly err: unknown) {}
}

export type EffResult<A> = A | EffError;

export type Arrs<E extends AnyEffect, A, B> =
  | Leaf<E, A, B>
  | Node<E, A, any, B>;

export class Leaf<E extends AnyEffect, A, B> {
  static ok<E extends AnyEffect, A, B>(
    f: <AA extends A>(x: AA) => Eff<E, B>
  ): Arrs<E, A, B> {
    return new Leaf<E, A, B>(r => {
      if (r instanceof EffError) {
  ...
  static err<E extends AnyEffect, A>(
    f: (err: unknown) => Eff<E, A>
  ): Arrs<E, A, A> {
  ...
  constructor(readonly f: <AA extends A>(x: EffResult<AA>) => Eff<E, B>) {}

  chain<F extends AnyEffect, C>(
    f: <BB extends B>(x: BB) => Eff<F, C>
  ): Arrs<E | F, A, C> {
    return this.concat(Leaf.ok(f));
  }

  catch<F extends AnyEffect>(
    f: (err: unknown) => Eff<F, B>
  ): Arrs<E | F, A, B> {
...
function createView<E extends AnyEffect, A, B>(
  f: Arrs<E, A, B>
): View<E, A, B> {
